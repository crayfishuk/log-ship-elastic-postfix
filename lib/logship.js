...
PostfixToElastic.prototype.populatePfdocsFromEs = function(done) {
  ...
  // API reference at https://www.elastic.co/guide/en/elasticsearch/client/javascript-api/5.x/api-reference.html#_search
    // 'terms' supports array https://github.com/toptal/chewy/issues/401
  this.elastic.search({
  ...
  function (err, res) {
    ...
    logger.info('\telastic doc matches: ' + res.hits.total.value);
...
PostfixToElastic.prototype.saveResultsToEs = function(done) {
  ...
  Object.keys(p2e.pfDocs).forEach(function (qid) {
    ...
    if (doc._parent && p2e.cfg.elastic.parent) {
      ...
      doc.type = p2e.cfg.elastic.parent.type;
    }
    else {
      bulkMeta._index  = p2e.cfg.elastic.index + '-' + dateSuffix;
      doc.type = p2e.cfg.elastic.type;
    }
  ...
  // logger.info(esBulk);
  p2e.elastic.bulk({ body: esBulk, waitForActiveShards: 1 }, function (err, res) {
    if (err) return done(err);
    if (res.errors) {
