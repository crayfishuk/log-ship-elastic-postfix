...
PostfixDoc.prototype.update = function (doc, lo) {
    switch (lo.prog) {
        ...
        case 'postfix/pickup':   // tells us the uid
            doc.uid = lo.uid;
            return;
        case 'postfix/smtpd': // May contain sasl user
            if (lo.sasl_username) {
                doc.username = lo.sasl_username;
            }
            break;
        case 'postfix/error':
            lo.action = 'error';
...
PostfixDoc.prototype.addToPostfixDocSmtp = function (doc, lo) {
    ...
    // Split status keyword off front of status message
    if (lo.status) {
        lo.status_key=lo.status.split(' ',1)[0];
    }
    // Split domain from the to field
    if (lo.to) {
        lo.to_domain=lo.to.split('@',2)[1];
    }
...
PostfixDoc.prototype.addToPostfixDocQmgr = function (doc, lo) {
    ...
    ['from', 'size', 'nrcpt'].forEach(function (field) {
        let domain = '';
        if (field == 'from') {
            // If we have a from field & domain can be split - save it separately
            domain = lo[field].split('@')
            if (domain[1]) {
                doc['from_domain'] = domain[1]
            }
        }
        doc[field] = lo[field];
        delete lo[field];
